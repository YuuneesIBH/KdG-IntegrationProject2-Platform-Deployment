# Team8 Kubernetes Deployment Guide
...
## 🏗️ Team8-Specific Architecture

Active manifests applied by `deploy-team8.sh` (in order):
...
03-game-blokus.yaml                  # Blokus services
...
# Cluster name output: see terraform outputs (e.g., bordspel-platform-team8)
```
...
2. Create new token with name "Team8-Kubernetes"
...
**What happens:**
1. Creates namespace `bordspelplatform`
...
api-gateway   LoadBalancer   10.0.1.100   34.79.116.98     80:30123/TCP   2m
```

**Access URLs:**
- **Platform**: `http://34.79.116.98/`
- **Keycloak**: `http://34.79.116.98/auth`
- **RabbitMQ Management**: `http://34.79.116.98:15672` (user/password)
- **Kibana Logs**: `http://34.79.116.98:5601` (if exposed)
...
```bash
# Check the secret
kubectl get secret gitlab-registry -n bordspelplatform -o yaml

# Re-create the secret
./setup-gitlab-registry.sh

# Check if pods can access registry
kubectl describe pod <pod-name> -n bordspelplatform | grep -A 10 Events
```
...
kubectl run -it --rm debug --image=postgres:latest --restart=Never \
  -n bordspelplatform -- \
  psql -h postgres-service.bordspelplatform.svc.cluster.local -U user -d postgres
...
kubectl run -it --rm debug --image=busybox --restart=Never \
  -n bordspelplatform -- \
  nslookup postgres-service.bordspelplatform.svc.cluster.local

# Test service connectivity
kubectl run -it --rm debug --image=busybox --restart=Never \
  -n bordspelplatform -- \
  wget -O- http://api-gateway/
```
...
# Describe a PVC
kubectl describe pvc <pvc-name> -n bordspelplatform
```
...
kubectl get configmap gateway-config -n bordspelplatform -o yaml

# Check gateway pod logs
kubectl logs -f deployment/api-gateway-deployment -n bordspelplatform

# Test from gateway pod
kubectl exec -it deployment/api-gateway-deployment -n bordspelplatform -- \
...
| **ImagePullBackOff** | GitLab credentials issue. Run `./setup-gitlab-registry.sh` |
| **LoadBalancer pending** | GCP might take 5-10 minutes. Check with: `kubectl get svc api-gateway -w` |
| **Database pod not ready** | Allow more time for initialization. Check: `kubectl logs postgres-deployment...` |
| **Services can't connect** | Check DNS: `kubectl run -it --rm debug --image=busybox --restart=Never -n bordspelplatform -- nslookup <service>` |
...
kubectl logs -f deployment/platform-backend-deployment -n bordspelplatform

# View logs from specific pod
kubectl logs <pod-name> -n bordspelplatform

# View previous crashed logs
kubectl logs <pod-name> -n bordspelplatform --previous
```
...
watch kubectl get pods -n bordspelplatform
```
...
# Follow events
kubectl get events -n bordspelplatform -w
```
...
# Monitor scaling
kubectl get pods -l app=platform-backend -n bordspelplatform -w
```
...
kubectl rollout history deployment/platform-backend-deployment -n bordspelplatform

# Rollback to previous version
kubectl rollout undo deployment/platform-backend-deployment -n bordspelplatform

# Rollback to specific revision
kubectl rollout undo deployment/platform-backend-deployment -n bordspelplatform --to-revision=2
```
...
```bash
# Edit ConfigMap
kubectl edit configmap platform-config -n bordspelplatform

# Edit Secret
kubectl edit secret platform-secrets -n bordspelplatform

# Pods will need restart to pick up changes
kubectl rollout restart deployment/<deployment-name> -n bordspelplatform
```
...
This will delete the entire `bordspelplatform` namespace and all its resources.
...
| `verify.sh` | Verify YAML configurations |
| `setup-gitlab-registry.sh` | Configure GitLab registry access |
